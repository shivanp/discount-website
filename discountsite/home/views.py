...
def index(request):
    ...
    if request.method == 'POST':
        master_array = [['bag2', 'https', 'https', '$45.00', '$25.00'], ['bag1', 'https', 'https', '$45.00', '$15.00']]
...
def sort(master_array):
    master_array = sorted(master_array, key=lambda x:(float(x[3][1:])-float(x[4][1:])/float(x[3][1:])), reverse=True)
    return master_array
